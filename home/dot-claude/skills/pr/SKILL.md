...
4. **Never include Linear issue IDs in the PR title or description.** Linear automatically links PRs via branch names, so mentioning the ID is redundant.
5. If args were passed (e.g. a Linear issue ID), include a link to it in the description only if it is NOT already in the branch name.
